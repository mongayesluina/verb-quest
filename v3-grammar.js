const Grammar=(()=>{
  ...
  function isThirdPerson(subject){return THIRD_PERSON_SUBJECTS.has(lowerSubject(subject)) || (!['i','you','we','they'].includes(lowerSubject(subject)) && !lowerSubject(subject).includes(' and '));}
  ...
  function present(v,subject){
    ...
    return isThirdPerson(subject)?present3(b):b;
  }
  ...
  function affirmative(v,tense='present',subject=null,marker=true){
    const c=context(v);const s=subject||c.subject;const form=tense==='past'?past(v,s):present(v,s);const time=marker?(tense==='past'?'yesterday':'every day'):'';
  ...
  function negative(v,tense='present',subject=null,marker=true){
    const c=context(v);const s=subject||c.subject;const time=marker?(tense==='past'?'yesterday':'every day'):'';let parts;
  ...
  function question(v,tense='present',subject=null,marker=true){
    const c=context(v);const s=subject||c.subject;const time=marker?(tense==='past'?'yesterday':'every day'):'';let parts;
    if(v.base==='be')parts=[tense==='past'?past(v,s):present(v,s),s,c.complement,time];
    else if(tense==='past')parts=['Did',s,v.base,c.complement,time];
    else parts=[isThirdPerson(s)?'Does':'Do',s,v.base,c.complement,time];
  ...
  function presentOptions(v,subject){
    const correct=present(v,subject);const distractors=new Set([v.base,present3(v.base),past(v,subject),v.participle.split('/')[0].trim()]);distractors.delete(correct);return [correct,...[...distractors].slice(0,3)];
  }
  function pastOptions(v,subject){
    const correct=past(v,subject);const distractors=new Set([v.base,present(v,subject),present3(v.base),v.participle.split('/')[0].trim()]);distractors.delete(correct);return [correct,...[...distractors].slice(0,3)];
  }
